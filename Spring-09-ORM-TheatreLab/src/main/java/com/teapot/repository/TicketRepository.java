...
import com.teapot.entity.Ticket;
...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    Integer countAllByMovieTheatreMovieName(String name);
    ...
    @Query(value = "SELECT DISTINCT(m.name) FROM ticket JOIN movie_theatre mt ON mc.id=ticket.movie_theatre_id JOIN movie m ON mc.movie_id=m.id",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ticket JOIN user_account ua ON ticket.user_account_id=ua.id " +
            ...
            " JOIN movie_theare mt ON ticket.movie_theatre_id=mt.id " +
