...
public interface TheatreRepository extends JpaRepository<Theatre, Long> {
    ...
    @Query(value = "SELECT DISTINCT(m.name) FROM ticket JOIN movie_Theatre mc ON mc.id=ticket.movie_theatreid JOIN movie m ON mc.movie_id=m.id",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ticket JOIN user_account ua ON ticket.user_account_id=ua.id " +
            ...
            " JOIN movie_theatre mt ON ticket.movie_theatre_id=mt.id " +
